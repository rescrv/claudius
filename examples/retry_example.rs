...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.send(params).await {
        ...
        Err(e) => {
            ...
            if e.is_rate_limit() {
                println!("This was a rate limit error - the client would have automatically retried with backoff");
            } else if e.is_retryable() {
                println!("This was a retryable error - the client would have automatically retried");
            } else {
