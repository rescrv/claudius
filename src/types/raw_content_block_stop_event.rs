...
impl RawContentBlockStopEvent {
    /// Create a new `RawContentBlockStopEvent` with the given index.
...
mod tests {
    ...
    #[test]
    fn test_raw_content_block_stop_event_serialization() {
        let event = RawContentBlockStopEvent::new(0);
    ...
    #[test]
    fn test_raw_content_block_stop_event_deserialization() {
        let json = json!({
            "index": 0,
            "type": "content_block_stop"
        });

        let event: RawContentBlockStopEvent = serde_json::from_value(json).unwrap();
